...
- SuperLightLogger 依存とライブラリ内部の独自ログ出力を削除し、失敗を呼び出し元へ例外として伝播するように変更。
- Ollama API の接続先切り替え、HTTP エラー、ストリーミング中のエラー通知を、再初期化後も呼び出し元へ一貫して伝播するように修正。
- Speech・Media の自動ダウンロード無効化、Piper 音声名の保存先検証、Linux 展開処理のキャンセル、MAUI の多重送信を堅牢化。
